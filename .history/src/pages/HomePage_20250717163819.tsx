...
import { debugUserTweets } from '../services/nitterService';
...
function HomePage({ selectedProject }: HomePageProps) {
  ...
  useEffect(() => {
    ...
    (window as any).debugUserTweets = debugUserTweets;
    
    console.log('🔧 Debug function added to window. Try: debugUserTweets("WojackLabs")');
  }, []);
  ...
  const loadData = async () => {
    try {
      ...
      // Add test users for debugging if no users found
      if (userList.length === 0) {
        const testUsers = ['WojackLabs', 'elonmusk', 'vitalikbuterin'];
        const testUserList = testUsers.map(handle => ({
      ...
      const uniqueHandles = Array.from(
        new Set(userList.length > 0 
          ? userList.map(user => user.twitterHandle) 
          : ['WojackLabs', 'elonmusk', 'vitalikbuterin']
        )
      );
      
      console.log('🔍 Loading tweets for users:', uniqueHandles);
      
      const tweets = await getEmbeddedTweetsForUsers(uniqueHandles.slice(0, 5)); // Limit to 5 users for performance
      ...
      {embeddedTweets.length > 0 && (
        <section className="tweets-section">
          ...
          <div className="debug-info">
            <p>
              <strong>Debug:</strong> Open browser console and try: <code>debugUserTweets("WojackLabs")</code>
            </p>
          </div>
          ...
          <div className="tweets-info">
            <p>
              <strong>Note:</strong> Testing automatic tweet fetching with Nitter RSS feeds.
            </p>
          </div>
        </section>
      )}
