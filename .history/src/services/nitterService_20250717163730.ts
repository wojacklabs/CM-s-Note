...
export async function fetchLatestTweetsFromNitter(username: string): Promise<string[]> {
  ...
  for (const instance of NITTER_INSTANCES) {
    ...
    const tweets = await tryNitterInstance(username, instance);
    
    if (tweets.length > 0) {
      console.log(`Successfully fetched ${tweets.length} tweets for ${username} from ${instance}`);
      return tweets;
    }
  }
  
  console.warn(`Failed to fetch tweets for ${username} from all instances`);
  return [];
}
...
export async function debugUserTweets(username: string): Promise<void> {
  console.log(`🔍 Debugging tweets for: ${username}`);
